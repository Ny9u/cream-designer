<template>
  <el-form-item label-width="0">
    ...
    <el-cascader
      placeholder="请选择"
      :options="widget.options.cascaderOptions"
...
const state = reactive({
  ...
  content: computed({
    get() {
      return JSON.stringify(props.optionModel.cascaderOptions, null, ' ')
    },
    set(v) {
      console.log(v)
    }
...
const importItemHandle = (v) => {
  state.optionModel.cascaderOptions = JSON.parse(v)
}
</script>
